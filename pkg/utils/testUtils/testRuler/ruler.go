...
// New return pointer on Ruler. This object is useful to test struct fields or slice order.
func New(t *testing.T) *Ruler {
	return &Ruler{t: t}
}
...
// CheckStruct compare all fields from actual interface with the expected ones from map.
// If new fields are added into struct but not in expected map, test will be failed.
// You will need to test also these new fields.
...
// SliceIsSorted check that slice is correctly sorted using sort package.
func (ruler *Ruler) SliceIsSorted(slice interface{}, less func(i, j int) bool) {
	require.True(ruler.t, sort.SliceIsSorted(slice, less), "slice is not correctly sorted")
}
